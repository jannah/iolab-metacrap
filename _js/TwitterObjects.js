...
function Tweet(/*tweetObj */) {
    ...
    if (arguments.length === 1)
    {
        ...
        this.hashtags = tweetObj.entities.hashtags;

        this.user_mentions = [];
        for (var i in tweetObj.entities.user_mentions)
        {
            var mention = new UserMentions(tweetObj.entities.user_mentions[i]);
...
function UserMentions(/* user_mentions object*/) {
    if (arguments.length === 1)
    {
...
function init() {
    ...
    {
        myTweets.push(new Tweet(tweets[tweet]));
    }
